import { hashPassword, verifyEmailInput, verifyPasswordHash, verifyPasswordStrength, verifyUsernameInput } from "lib/authentication";
...
export abstract class UserService {
     ...
     */
    static async authenticateUser(usernameOrEmail: string, password: string): Promise<Result<SelectUser>> {
        usernameOrEmail = usernameOrEmail.trim();
        let user: SelectUser | undefined;
        if (verifyUsernameInput(usernameOrEmail)) {
            user = await db.selectFrom("user").where("username", "=", usernameOrEmail).selectAll().executeTakeFirst();
        } else if (verifyEmailInput(usernameOrEmail)) {
            user = await db.selectFrom("user").where("email", "=", usernameOrEmail).selectAll().executeTakeFirst();
        } else {
        ...
        const credential = await db.selectFrom("credential").where("userId", "=", user.id).select("passwordHash").executeTakeFirst();
